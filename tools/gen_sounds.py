...
def gen_boom(filename, duration=0.35):
    sample_rate = 22050
    num_samples = int(sample_rate * duration)
    ...
    with wave.open(filename, "w") as wav:
        ...
        for i in range(num_samples):
            t = i / sample_rate
            freq1 = 120 - t * 80       # 120→40Hz
            freq2 = 200 - t * 160      # 200→40Hz harmonic
            amplitude = math.exp(-t * 5) * 1.2 + math.exp(-t * 15) * 0.5
            sine1 = math.sin(2 * math.pi * freq1 * t)
            sine2 = math.sin(2 * math.pi * freq2 * t + 1.2)
            noise = (random.random() - 0.5) * math.exp(-t * 8) * 0.6
            sample = int((sine1 * 0.7 + sine2 * 0.3 + noise) * amplitude * 20000)
            sample = max(-32768, min(32767, sample))
            wav.writeframes(struct.pack("<h", sample))
...
gen_boom("app/src/main/res/raw/boom.wav")
